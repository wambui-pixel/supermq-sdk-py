...
config = {
   "external_id": "234567",
  "external_key": "234567",
  "client_id": "828b93e3-52b3-43a4-9ce2-ed8a47127ddd",
  "name": "demo5"
}
...
def test_add(requests_mock):
    requests_mock.register_uri( "POST", url+ "/clients/configs", headers={"location": "/configs/" + client_id}, json=config, status_code=201)
...
def test_add_bad_token(requests_mock):
    requests_mock.register_uri( "POST", url+ "/clients/configs", headers={"location": "/configs/" + client_id}, json=config, status_code=401)
    r = s.bootstrap.add(config=config, token=token)
    assert r.error.status == 1
    assert r.error.message =="Missing or invalid access token provided."
    
def test_whitelist(requests_mock):
    requests_mock.register_uri( "PUT", url+ "/clients/state/" + config["client_id"], json=config, status_code=201)
...
def test_whitelist_bad_config(requests_mock):
    requests_mock.register_uri( "PUT", url+ "/clients/state/" + config["client_id"], json=config, status_code=400)
    r = s.bootstrap.whitelist(config=config, token=token)
    assert r.error.status == 1
    assert r.error.message == "Failed due to malformed config's ID."

def test_whitelist_config_removed(requests_mock):
    requests_mock.register_uri( "PUT", url+ "/clients/state/" + config["client_id"], json=config, status_code=204)
    r = s.bootstrap.whitelist(config=config, token=token)
    assert r.error.status == 1
    assert r.error.message == "Config removed."
        
def test_view(requests_mock):
    requests_mock.register_uri( "GET", url+ "/clients/configs/" + client_id, json=config, status_code=200)
    r = s.bootstrap.view(client_id=client_id, token=token)
    assert r.error.status == 0
    assert config == r.value

def test_view_bad_config(requests_mock):
    requests_mock.register_uri( "GET", url+ "/clients/configs/" + client_id, json=config, status_code=404)
    r = s.bootstrap.view(client_id=client_id, token=token)
    assert r.error.status == 1
    assert r.error.message == "Config does not exist."
    
def test_update(requests_mock):
    requests_mock.register_uri( "PUT", url+ "/clients/configs/" + config["client_id"], json=config, status_code=200)
    r = s.bootstrap.update(config=config, token=token)
    assert r.error.status == 0
    assert r.value == "Configuration updated."

def test_update_bad_config(requests_mock):
    requests_mock.register_uri( "PUT", url+ "/clients/configs/" + config["client_id"], json=config, status_code=404)
    r = s.bootstrap.update(config=config, token=token)
    assert r.error.status == 1
    assert r.error.message == "Config does not exist."
