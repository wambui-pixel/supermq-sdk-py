...
certs= {
  ...
  "client_id": "3d49a42f-63fd-491b-9784-adf4b64ef347"
}
...
client_id=  "2544670a-a992-4865-8222-24e5deed7bae"
cert_id= "22:16:df:60:c2:99:bc:c4:9b:1d:fd:71:5e:e9:07:d9:1b:3c:85:1d"

url= "http://localhost"

def test_issue(requests_mock):
    requests_mock.register_uri( "POST", url + "/certs", headers={"location": "/certs/" + client_id}, json=certs, status_code=201)
    r = s.certs.issue(client_id=client_id, valid= "10h", token=token)
    assert r.error.status == 0
    assert certs == r.value
    
def test_issue_bad_token(requests_mock):
    requests_mock.register_uri( "POST", url + "/certs", headers={"location": "/certs/" + client_id}, json=certs, status_code=401)
    r = s.certs.issue(client_id=client_id, valid= "10h", token=token)
...
def test_revoke(requests_mock):
    requests_mock.register_uri( "DELETE", url + "/certs" + "/" + client_id, status_code=200)
    r = s.certs.revoke(client_id=client_id, token=token)
    assert r.error.status == 0
    
def test_revoke_bad_response(requests_mock):
    requests_mock.register_uri( "DELETE", url + "/certs" + "/" + client_id, status_code=404)
    r = s.certs.revoke(client_id=client_id, token=token)
    assert r.error.status == 1
